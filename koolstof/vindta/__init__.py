...
def addfunccols(df, func, *args):
    ...
    return pd.concat(
        [df, df.apply(lambda x: func(x, *args), axis=1)], axis=1, sort=False
    )
...
def _logfile2dbs(x, logfile):
    if x.bottle in logfile.bottle.values:
        xix = np.where(
            (x.bottle == logfile.bottle) & (x.analysisdate == logfile.analysisdate)
        )[0]
        assert len(xix) == 1, (
...
def _get_blanks(x, logfile, usefrom):
    assert "logfile_iloc" in x.index, "You must first run `ks.vindta.logfile2dbs()`."
