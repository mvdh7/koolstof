...
def pH_optode(spreadsheet_path, text_files_folder_path, cutoff=1200, end_mins=120):
    ...
    for file in file_list:
        L = data[file].sec > data[file].sec.max() - end_mins
        results.loc[results.filename == file, "pH_end"] = data[file][
            L
        ].pH.mean()
        results.loc[results.filename == file, "pH_end_median"] = data[file][
            L
        ].pH.median()
        ...
        results.loc[results.filename == file, "pH_NBS_raw"] = data[file].pH.mean()

        # Store the median (raw pH) in df
        results.loc[results.filename == file, "pH_NBS_raw_median"] = data[file].pH.median()
