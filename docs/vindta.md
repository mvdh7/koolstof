...
To find and apply the blank corrections, use `ks.blank_correction`.
...
```python
ks.plot_increments(dbs, logfile, use_from=6)
```

This generates a figure like below:

![Output from ks.plot_increments](https://raw.githubusercontent.com/mvdh7/koolstof/main/docs/img/plot_increments.png)
...
![Output from ks.plot_blanks](https://raw.githubusercontent.com/mvdh7/koolstof/main/docs/img/plot_blanks.png)
...
If any points fall far away from the rest and are causing the solid line to not have a good fit to the data, you can ignore them (in this curve fitting step only) by setting the `"blank_good"` column of `dbs` to `False` for those rows.  Then, re-run `ks.blank_correction`.  These points will subsequently show up as open symbols on these plots (see legend - 'Ignored') and won't influence the fitted line.
...
```python
ks.plot_k_dic(dbs, sessions, show_ignored=True)
```

The plot may look something like this:

![Output from ks.plot_k_dic()](https://raw.githubusercontent.com/mvdh7/koolstof/main/docs/img/plot_k_dic_0.png)
...
In the example above, there are clearly two bad CRM measurements.  You can exclude these from the calibration by setting `"k_dic_good"` to false, then re-run `ks.calibrate_dic` to recalibrate.  Recreating the figure above but with `show_ignored=False` now gives us a clearer picture of the calibrations:

![Output from ks.plot_k_dic()](https://raw.githubusercontent.com/mvdh7/koolstof/main/docs/img/plot_k_dic_1.png)
...
![Output from ks.plot_dic_offset](https://raw.githubusercontent.com/mvdh7/koolstof/main/docs/img/plot_dic_offset.png)
